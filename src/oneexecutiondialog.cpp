...
void OneExecutionDialog::selectExercise(QListWidgetItem* item)
{
    ...
    QStringList paramNames, paramValues, paramUnits, comments;
    QString comment, lastComment;
    _db.fetchExInfo(id, paramNames, paramValues, paramUnits, comment, comments, lastComment);
    ...
    for (int i = 0; i < paramNames.count(); ++i)
    {
        ...
        params->setItem(i, 1, new QTableWidgetItem(paramValues[i]));
    ...
    thisComment->setEditText(lastComment);
}
