    ...
    : _db(QSqlDatabase::addDatabase("QSQLITE"))
{
    ...
    if (!_db.open())
    {
        ...
        QMessageBox msgBox;
        msgBox.setText("Cannot open database " + QDir::currentPath() + "/" + QString(DbName) + "\n\n" + _db.lastError().text());
        msgBox.exec();
    }
...
void Database::fetchExInfo(int id, QStringList &paramNames, QStringList& paramValues, QStringList &paramUnits, QString &comment, QStringList &comments, QString &lastComment)
{
    QSqlQuery query;
    ...
    if(query.next())
    {
        paramsCount = query.value(0).toInt();
    ...
    query.prepare(SelectExLastUsage);
    query.bindValue(":id", id);
    query.exec();
    if(query.next())
    {
        lastComment = query.value(0).toString();
        for (int i = 0; i < paramsCount; ++i)
        {
            paramValues << query.value(i + 1).toString();
        }
    }
    else
    {
...
uint Database::fetchDayAverallTimes(const QDate date)
{
    uint result = 0;
    QSqlQuery query;
    query.prepare(SelectDiaryDayOverallTime);
    query.bindValue(":date", toDelphiDate(date));
    query.exec();
    if(query.next())
    {
