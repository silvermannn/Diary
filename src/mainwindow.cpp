...
void MainWindow::updateExercises(QDate date)
{
    ...
    uint overallTime = _db.fetchDayAverallTimes(date);

    QLabel* total = findChild<QLabel*>("totalTime");
    total->setText(QString("Суммарное время %1:%2.\tВсего упражнений: %3.").arg(
        QString::number(overallTime / 60),
        QString::number(overallTime % 60).rightJustified(2, '0'),
        QString::number(es.count())));
